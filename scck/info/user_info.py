...
def generate_slurm_info():
    ...
    try:
        ...
        perm = subprocess.run(
            ["sacctmgr", "show", "assoc", "user=" +
                current_user, "format=partition,QOS"],
            capture_output=True, text=True, check=True
        )
        ...
        perm = list(map(lambda x: x.strip().split(),
                        perm.stdout.strip().split("\n")[2:]))
        ...
        for i, line in enumerate(lines):
            ...
            if line[0].upper() in available_partitions:
                ...
                partitions[line[0]] = {
                    ...
                    "CPUS": cpus_per_node,
                    "GPUS": gpu_count,
                    "QOS": qos,  # Would need additional command to get QOS
